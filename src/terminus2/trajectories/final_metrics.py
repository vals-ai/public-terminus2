...
class FinalMetrics(BaseModel):
    ...
    total_prompt_tokens: int | None = Field(
        default=None,
        description="Sum of all prompt tokens across all steps (excluding cache read tokens)",
    )
    total_completion_tokens: int | None = Field(
        default=None,
        description="Sum of all completion tokens across all steps (excluding reasoning tokens)",
    )
    total_reasoning_tokens: int | None = Field(
        default=None,
        description="Sum of all reasoning/thinking tokens across all steps",
    )
    total_cache_read_tokens: int | None = Field(
        default=None,
        description="Sum of all cache read tokens across all steps",
    )
    total_cache_write_tokens: int | None = Field(
        default=None,
        description="Sum of all cache write tokens across all steps",
    )
